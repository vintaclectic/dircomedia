...
| PM2 services | ✅ **6 online** (re-measured 2026-08-19, W84PNK2, after HG5GCNB restore) — all declared in `ecosystem.config.js`, so a reboot no longer loses the stack. | `dircomedia-api`, `-gateway`, `-frontend`, `-shim`, `-worker`, `-tunnel` |
| **LIVE PUBLIC URL** | ✅ **UP** (re-measured 2026-08-19, W84PNK2). Public visitors get **403 `DirCoMedia — locked`** — that is the gateway lock working as designed (`gateway.js:109`), **not** an outage. Owner lane verified **200**. `/__gateway/health` → **200**. | **https://dircomedia.vintaclectic.com** → dashboard HTTP **200**, 64,086 bytes, `<title>DirCo Media OS</title>` |
